...
export const SideBarMenuItem = ({ item }: { item: SideNavItem }) => {
    ...
    const inactiveLink = classNames("flex items-center min-h-[40px] h-full text-white py-2 px-4 hover:text-white hover:bg-red-700 rounded-md transition duration-200",
        { ["justify-center"]: toggleCollapse }
    );

    const activeLink = classNames(" text-white bg-red-700");

    const navMenuDropdownItem = "text-red py-2 px-4 hover:text-white transition duration-200 rounded-md"

    const dropdownMenuHeaderLink = classNames(inactiveLink,
        {
            ["bg-red-700 rounded-b-none"]: subMenuOpen
        }
    );
    return (
        <>
            {item.submenu ? (
                <div className="min-w-[18px]">
                    ...
                    {subMenuOpen && !toggleCollapse && (
                        <div className='bg-red-800 border-l-4'>
